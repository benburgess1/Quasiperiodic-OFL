...
def calc_phi_vs_a(a_vals, dx_vals=None, save=True, filename='Data.npz', **kwargs):
    phi_vals = np.zeros(a_vals.size)
    if dx_vals is None:
        dx_vals = np.ones(a_vals.size)
    for i, a in enumerate(a_vals):
        print(f'Evaluating a = {a} ({i+1} out of {a_vals.size})')
        dx = dx_vals[i]
        x_vals = np.arange(0, 2*np.pi*a, dx)
        y_vals = np.copy(x_vals)
        B_eff = calc_B_eff(x_vals=x_vals, y_vals=y_vals, a=a, save=False, **kwargs)
        phi_vals[i] = np.sum(B_eff)
        print('\n')
    if save:
        np.savez(filename, a_vals=a_vals, phi_vals=phi_vals, dx_vals=dx_vals, **kwargs)
    else:
        return phi_vals
...
def plot_phi_vs_a(filename, average_uc=True, 
    ...
    if average_uc:
        ...
        ylab = r'$\Phi$'
    fig,ax = plt.subplots()
    ax.plot(a, phi, color='b', marker='x', ls='-')
    ax.set_xlabel(r'$a$')
    ax.set_ylabel(ylab)
    title_str = ylab
